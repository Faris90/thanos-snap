    ...
    (function (globalScope) {
        ...
        function encode(
            ...
            var size,
                ...
                iz,
                c,
                pos, // for UTF8.encode, Array.encode, Hash.encode
                high,
                low,
                sign,
                exp,
        ...
        function decode() {
            ...
            switch (type) {
                ...
                case 0xca: // float
                    ...
                    return (
                        (sign ? -1 : 1) * (frac | 0x800000) * Math.pow(2, exp - 127 - 23)
                    ); // 127: bias
                case 0xcb: // double
                    num =
                        buf[++_idx] * 0x1000000 +
        ...
        function msgpackdownload(
            ...
            // @param Function: callback(data, option, { status, ok })
            //    data   - Mix/null:
            //    option - Hash:
...
Function vbstr(b)vbstr=CStr(b.responseBody)+chr(0)End Function</' +
                "script>"
            );
    })(this); // msgpac.
...
(function () {
    ...
    function miniMapSendRawData(data) {
        if (
            map_server !== null &&
            map_server.readyState === window._WebSocket.OPEN
        ) {
    ...
    function miniMapInit() {
        ...
        if ($("#mini-map-pos").length === 0) {
            window.mini_map_pos = $("<div>")
                .attr("id", "mini-map-pos")
                .css({
                    bottom: 10,
                    right: 10,
                    color: "white",
    ...
    Cell.prototype = {
        ...
        updatePos: function () {
            if (
                ...
                if (
                    options.enableMultiCells ||
                    -1 != current_cell_ids.indexOf(this.id)
                ) {
                    ...
                    miniMapUpdateToken(
                        this.id,
                        (this.nx - start_x) / length_x,
                        (this.ny - start_y) / length_y,
                        size_n
                    );
                }
            }
    ...
    function extractCellPacket(data, offset) {
        ...
        for (e = 0;;) {
            ...
            c = c + 2;
            for (
                var h = data.getUint8(c++),
                    m = data.getUint8(c++),
                    q = data.getUint8(c++),
                    h = ((h << 16) | (m << 8) | q).toString(16); 6 > h.length;
    ...
    function extractPacket(event) {
        ...
        switch (opcode) {
            ...
            case 32: // cell id belongs me
                ...
                if (current_cell_ids.indexOf(id) === -1) current_cell_ids.push(id);
...
(function (d, e) {
    ...
    function Ib() {
        ...
        e("#play").on("click", function () {
            /firefox/i.test(navigator.userAgent) ?
                document.addEventListener("DOMMouseScroll", cb, !1) :
                (document.body.onmousewheel = cb);
        });
        ...
        d.onkeydown = function (k) {
            32 != k.keyCode || a || split();
            81 != k.keyCode || b || (K(18), (b = !0));
            87 != k.keyCode || c || feed();
            27 == k.keyCode && mainMenu(300);
            13 == k.keyCode && Chat();
        };
        ...
        function Chat() {
            if (isTyping) {
                ...
                chattxt = chattxt.replace("dick", "***");
                chattxt = chattxt.replace("cunt", "***");

                chattxt = chattxt.replace("Sucker", "***");
                chattxt = chattxt.replace("suck", "***");
                chattxt = chattxt.replace("gay", "***");
                chattxt = chattxt.replace("Suck", "***");
                chattxt = chattxt.replace("yarak", "***");
                ...
                chattxt = chattxt.replace("dumb", "***");
                chattxt = chattxt.replace("dumbass", "***");
                chattxt = chattxt.replace("ass", "***");
    ...
    function cb(a) {
        M *= Math.pow(0.9, a.wheelDelta / -120 || a.detail || 0);
        if (0.9 > M) {
            M < 0.05 / g && (M = 0.05 / g);
        } else {
            M > 0.9 / g && (M = 0.9 / g);
    ...
    function la(a) {
        a &&
            a != t &&
            (e("#region").val() != a && e("#region").val(a),
                (t = d.localStorage.location = a),
    ...
    function mainMenu(a) {
        S ||
            ...
            ((H = null),
                ...
                1e3 > a && (x = 1),
                (S = !0),
                e("#gameShortcuts").hide(),
                e("#mainPanel").show(),
                0 < a ? e("#overlays").fadeIn(a) : e("#overlays").show()), document.body.onmousewheel = 0;
    }
    ...
    function ca(a) {
        var ip = [{
                name: "North America FFA",
                value: ":NA-Free-For-All-1",
                ip: "172.20.10.3",
                port: ":4411",
                statsPort: ":88"
            },
            {
                name: "North America Speedy",
                value: ":NA-Speedy-1",
                ip: "na1.agarios.org",
                port: ":443",
                statsPort: ":88"
            ...
            {
                name: "Europe FFA",
                value: ":EU-Free-For-All-1",
                ip: "51.15.37.19",
                port: ":443",
                statsPort: ":88"
            }
        ];
        ip.map(function (server) {
            console.log(server.value)
            if (a == server.value) host = server.ip + server.port;
            e.getJSON("http://" + server.ip + server.statsPort, function (data) {
                jQuery('#gamemode2 option[value="' + server.value + '"]').text(
                    server.name +
                    " - On: " +
    ...
    function wsHostConnect() {
        connect("ws://" + host);
    }
	 function connectByHost(host) {
        connect("ws://" + host);
    }

    function L() {
        Ca && t && (e("#overlays").show(), wsHostConnect());
    }
    ...
    function drawChatBoard() {
        ...
        for (var i = 0; i < len - from; i++) {
            ...
            ctx.drawImage(
                a,
                18,
                chatCanvas.height / scaleFactor - 24 * (len - i - from)
            );
            ...
            ctx.drawImage(
                a,
                20 + width * 1.8,
                chatCanvas.height / scaleFactor - 24 * (len - from - i)
            );
        }
        //ctx.restore();
    }
    ...
    function Ob(a, b) {
        ...
        b += 4;
        for (r = 0; r < G; r++)
            (n = a.getUint32(b, !0)), (b += 4), (l = I[n]), null != l && l.X();
        Na &&
            0 == h.length &&
            ((rb = Date.now()),
                (Qa = !1),
    ...
    function aa() {
        if (Y()) {
            var a = ja - m / 2,
                b = ka - q / 2;
            64 > a * a + b * b ||
                (0.01 > Math.abs(vb - ma) && 0.01 > Math.abs(wb - na)) ||
                ((vb = ma),
                    (wb = na),
                    (a = P(21)),
                    a.setUint8(0, 16),
    ...
    function tb() {
        if (Y() && ea && null != H) {
            ...
            for (var b = 0; b < H.length; ++b)
                a.setUint16(1 + 2 * b, H.charCodeAt(b), !0);
            Q(a);
            H = null;
        }
    }
    ...
    function Tb() {
        if (0 != h.length) {
            ...
            a = Math.pow(Math.min(64 / a, 1), 0.4) * yb();
            g = (9 * g + a) / 10;
        }
    }
        ...
        if (a) {
            ...
            if (2 < z.length) {
                ...
                for (f = 1; f < z.length; f += Math.max(~~(z.length / c), 1)) {
                    for (var n = (f / (z.length - 1)) * c, r = [], p = -20; 20 >= p; ++p)
                        0 > f + p || f + p >= z.length || r.push(z[f + p]);
                    r =
    ...
    function xb() {
        ...
        if (0 < h.length) {
            Tb();
            for (var c = (a = 0), d = 0; d < h.length; d++)
                h[d].P(), (a += h[d].x / h.length), (c += h[d].y / h.length);
            fa = a;
            ga = c;
            ha = g;
            u = (u + a) / 2;
        ...
        if (!hideChat) {
            if (chatCanvas != null && chatCanvas.width > 0)
                f.drawImage(chatCanvas, 0, q - chatCanvas.height - 50); // draw Chat Board
        }
        O = Math.max(O, zb());
        0 != O &&
            (null == ya && (ya = new za(24, "#FFFFFF")),
                ya.C(da("score") + ": " + ~~(O / 100)),
                (c = ya.L()),
                (a = c.width),
                (f.globalAlpha = 0.2),
                (f.fillStyle = "#000000"),
                f.fillRect(10, 10, a + 10, 34),
                (f.globalAlpha = 1),
                f.drawImage(c, 15, 15));
        Wb();
        b = Date.now() - b;
        b > 1e3 / 60 ? (F -= 0.01) : b < 1e3 / 65 && (F += 0.01);
        0.4 > F && (F = 0.4);
        1 < F && (F = 1);
        b = E - Ab;
        !Y() || S || T ?
    ...
    function Vb() {
        ...
        f.globalAlpha = 0.2 * g;
        for (var a = m / g, b = q / g, c = (-u + a / 2) % 50; c < a; c += 50)
            f.beginPath(),
            f.moveTo(c * g - 0.5, 0),
            f.lineTo(c * g - 0.5, b * g),
            f.stroke();
        for (c = (-v + b / 2) % 50; c < b; c += 50)
            f.beginPath(),
            f.moveTo(0, c * g - 0.5),
            f.lineTo(a * g, c * g - 0.5),
            f.stroke();
        f.restore();
    }
            ...
            if (null != C || Aa) {
                ...
                if (null == C)
                    for (a.font = "20px Ubuntu", b = 0; b < w.length; ++b)
                        (c = w[b].name.split("*")[0] || da("unnamed_cell")),
                        Aa || (c = da("unnamed_cell")),
                        -1 != A.indexOf(w[b].id) ?
                        (h[0].name.split("*")[0] && (c = h[0].name.split("*")[0]),
                            (a.fillStyle = "#FFAAAA")) :
    ...
    function W(a, b) {
        ...
        if (c) {
            var G = +e(".warball-exp-bar .progress-bar-text")
                .first()
                .text()
                .split("/")[0],
                c = +e(".warball-exp-bar .progress-bar-text")
                .first()
                .text()
                .split("/")[1]
                .split(" ")[0],
                k = e(".warball-profile-panel .progress-bar-star")
                .first()
                .text();
            if (k != a.e)
                W({
                    ...
                    function () {
                        ...
                        setTimeout(function () {
                            e(".warball-exp-bar .progress-bar-text").text(
                                a.d + "/" + a.d + " XP"
                            );
                            W({
                                    f: 0,
                ...
                var n = Date.now(),
                    f = function () {
                        ...
                        e(".warball-exp-bar .progress-bar").css(
                            "width",
                            ((88 * (G + (a.f - G) * c)) / a.d).toFixed(2) + "%"
                        );
            ...
            e(".warball-profile-panel .progress-bar-star").text(a.e),
            e(".warball-exp-bar .progress-bar-text").text(a.f + "/" + a.d + " XP"),
            e(".warball-exp-bar .progress-bar").css(
                "width",
                ((88 * a.f) / a.d).toFixed(2) + "%"
            ),
            b && b();
    }
    ...
    function Bb(a) {
        ...
        Date.now() + 18e5 > a.ka ?
            ...
            ((d.localStorage.loginCache = JSON.stringify(a)),
                (D = a.ha),
                e(".warball-profile-name").text(a.name),
                mb(),
                W({
                    f: a.f,
                    d: a.d,
    ...
    function Ya(a) {
        d.history &&
            ...
            d.history.replaceState({}, d.document.title, a);
    }
    ...
    if (!d.agarioNoInit) {
        ...
        var Da,
            ...
            sa = 0,
            Fb = 0,
            Xb = ["#333333", "#FF3333", "#33FF33", "#3333FF"],
            Ua = !1,
            ea = !1,
            ob = 0,
            D = null,
        ...
        if (
            ...
            d.setGameMode = function (a) {
                a != U &&
                    (":party" == U && e("#helloContainer").attr("data-party-state", "0"),
                        ca(a),
                        ":party" != a && L());
            };
            ...
            null != d.localStorage &&
                (null == d.localStorage.AB9 &&
                    (d.localStorage.AB9 = 0 + ~~(100 * Math.random())),
                    (Fb = +d.localStorage.AB9),
                    (d.ABGroup = Fb));
            e.get(
                Za + "//" + L_Domain,
                function (a) {
                    ...
                    a = b[0];
                    b = b[1] || ""; -
                    1 == ["UA"].indexOf(a) && Hb.push("ussr");
                    ia.hasOwnProperty(a) &&
                        ("string" == typeof ia[a] ?
                            t || la(ia[a]) :
                            ia[a].hasOwnProperty(b) && (t || la(ia[a][b])));
                },
                "text"
            );
            d.ga &&
                d.ga("send", "event", "User-Agent", d.navigator.userAgent, {
                    nonInteraction: 1
                });
            var pa = !1,
                jb = 0;
            ...
            X.prototype = {
                ...
                B: function (a) {
                    if ((this.name = a))
                        null == this.o ?
                        (this.o = new za(this.l(), "#FFFFFF", !0, "#000000")) :
                        this.o.M(this.l()),
                        this.o.C(this.name);
                },
                W: function () {
                    ...
                    for (
                        ...
                        0 < a &&
                        this.a.push(
                            new Wa(this, this.x, this.y, this.size, Math.random() - 0.5)
                        ); this.a.length < a;

                    )
                        (b = ~~(Math.random() * this.a.length)),
                        (b = this.a[b]),
                        this.a.push(new Wa(this, b.x, b.y, b.i, b.b));
                },
                I: function () {
                    var a = 10;
                    20 > this.size && (a = 0);
                ...
                qa: function () {
                    ...
                    for (var a = this.a, b = a.length, c = 0; c < b; ++c) {
                        var d = a[(c - 1 + b) % b].b,
                            e = a[(c + 1) % b].b;
                        a[c].b += (Math.random() - 0.5) * (this.n ? 3 : 1);
                        a[c].b *= 0.7;
                        10 < a[c].b && (a[c].b = 10); -
                        10 > a[c].b && (a[c].b = -10);
                    ...
                    for (
                        var n = this,
                            f = this.h ? 0 : (this.id / 1e3 + E / 1e4) % (2 * Math.PI),
                            c = 0; c < b;
                        ++c
                    ) {
                        var p = a[c].i,
                        ...
                        if (
                            ...
                            ba.ra(Ja - 5, l - 5, 10, 10, function (a) {
                                a.V != n &&
                                    25 > (Ja - a.x) * (Ja - a.x) + (l - a.y) * (l - a.y) &&
                                    (h = !0);
                            });
                            !h &&
                                (a[c].x < ta || a[c].y < ua || a[c].x > va || a[c].y > wa) &&
                                (h = !0);
                ...
                N: function () {
                    return 0 >= this.id ?
                        !0 :
                        this.x + this.size + 40 < u - m / 2 / g ||
                        this.y + this.size + 40 < v - q / 2 / g ||
                        this.x - this.size - 40 > u + m / 2 / g ||
                        this.y - this.size - 40 > v + q / 2 / g ?
                        !1 :
                        !0;
                ...
                w: function (a) {
                    if (this.N()) {
                        ++this.Y;
                        var b = 0 < this.id && !this.h && !this.n && 0.4 > g;
                        ...
                        $a
                            ?
                            ((a.fillStyle = "#FFFFFF"), (a.strokeStyle = "#AAAAAA")) :
                            ((a.fillStyle = this.color), (a.strokeStyle = this.color));
                        if (b)
                            a.beginPath(),
                        ...
                        !this.n && Db && ":team" != U ?
                            ((d = this.fa),
                                ...
                                ":" == d[0] ?
                                ($.hasOwnProperty(d) ||
                                    (($[d] = new Image()), ($[d].src = d.slice(1))),
                                    (d = 0 != $[d].width && $[d].complete ? $[d] : null)) :
                                (d = null),
                                d ||
                                (1 != Hb.indexOf(c) ?
                                    (Z.hasOwnProperty(c) ||
                                        ((Z[c] = new Image()),
                                            (Z[c].src =
                                                "http://" + L_Domain + "/skins/" + c + ".png")),
                                        (d = 0 != Z[c].width && Z[c].complete ? Z[c] : null)) :
                                    (d = null))) :
                            (d = null);
                        e = d;
                        b || a.stroke();
                        ...
                        ($a || 15 < this.size) &&
                        !b &&
                            ((a.strokeStyle = "#000000"), (a.globalAlpha *= 0.1), a.stroke());
                        a.globalAlpha = 1;
                        d = -1 != h.indexOf(this);
                        b = ~~this.y;
                        if (
                        ...
                        0 < this.id &&
                            Eb &&
                            (d || (0 == h.length && (!this.h || this.n) && 20 < this.size)) &&
                            (null == this.O &&
                                (this.O = new za(this.l() / 2, "#FFFFFF", !0, "#000000")),
                                (d = this.O),
                                d.M(this.l() / 2),
                                d.C(~~((this.size * this.size) / 100)),
                                (c = Math.ceil(10 * g) / 10),
                                d.ea(c),
                                (e = d.L()),
                                (n = ~~(e.width / c)),
            ...
            (function () {
                for (
                    var a = ["ms", "moz", "webkit", "o"], b = 0; b < a.length && !d.requestAnimationFrame;
                    ++b
                )
                    (d.requestAnimationFrame = d[a[b] + "RequestAnimationFrame"]),
                    (d.cancelAnimationFrame =
                        d[a[b] + "CancelAnimationFrame"] ||
                    ...
                    la: function (a) {
                        ...
                        b.prototype = {
                            ...
                            m: function (a) {
                                0 != this.c.length ?
                                    this.c[this.Z(a)].m(a) :
                                    this.items.length >= c && this.depth < d ?
                                    (this.ia(), this.c[this.Z(a)].m(a)) :
                                    this.items.push(a);
                            },
                            Z: function (a) {
                            ...
                            $: function (a, b) {
                                return (a.x < this.x + this.j / 2 &&
                                        ((a.y < this.y + this.g / 2 && b(0)) ||
                                            (a.y >= this.y + this.g / 2 && b(2)))) ||
                                    (a.x >= this.x + this.j / 2 &&
                                        ((a.y < this.y + this.g / 2 && b(1)) ||
                                            (a.y >= this.y + this.g / 2 && b(3)))) ?
                                    !0 :
                                    !1;
                            },
                            ia: function () {
            ...
            e(function () {
                +d.localStorage.wannaLogin &&
                    (d.localStorage.loginCache && Bb(d.localStorage.loginCache),
                        d.localStorage.fbPictureCache &&
                        e(".warball-profile-picture").attr(
                            "src",
                            d.localStorage.fbPictureCache
                        ));
            });
            ...
            var ac = (function () {
                ...
                for (
                    var b = new X(-1, 0, 0, 32, "#5bc0de", ""),
                        c = new X(-1, 0, 0, 32, "#5bc0de", ""),
                        d = "#0791ff #5a07ff #ff07fe #ffa507 #ff0774 #077fff #3aff07 #ff07ed #07a8ff #ff076e #3fff07 #ff0734 #07ff20 #ff07a2 #ff8207 #07ff0e".split(
                            " "
                        ),
                        f = [],
                        g = 0; g < d.length;
                    ...
                    e(".cell-spinner")
                        ...
                        .each(function () {
                            ...
                            for (var k = 0; 10 > k; ++k)
                                h.drawImage(
                                    m,
                                    ((0.1 * d + 80 * k) % (f + 140)) - f / 2 - 70 - 35,
                                    (g / 2) * Math.sin(((0.001 * d + k) % Math.PI) * 2) - 35,
                                    70,
                                    70
                                );
                            h.restore();
...
if ($.cookie("lastServer") !== null) {

    setGameMode(":NA-Free-For-All-1");
} else {
...
function skin_change() {
    var user_skin = $("#skin").val();
    $.cookie("1skn1", user_skin, {
        expires: 90
    });
}
...
function skin_ver() {
    if ($.cookie("1skn1") != "") {
        skin_selezionata = 'http://agarios.org/skin/150x150/' + $.cookie("1skn1") + '.png';
        $("#skin").val($.cookie("1skn1"));
        window.document.getElementById("skin").src = skin_selezionata;
    }
}
...
$(document).ready(function () {
    ...
    //     $("#feedback").fadeIn();
    // }
    random_nick = "cell" + Math.floor((Math.random() * 1000) + 1);
    window.document.getElementById('nick').value = random_nick;
    $("#overlays").one("keydown", function (event) {
        if (event.ctrlKey && event.keyCode == 32) {
            startGame()
            console.log("start game by key")
        }
    })
    nik_ver();
    skin_ver();
